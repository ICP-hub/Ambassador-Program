...
import { Stack, Typography, Button, Dialog, DialogTitle, DialogContent, DialogActions, DialogContentText, TextField, Select, MenuItem, Box, IconButton, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, } from '@mui/material';
...
const BalanceList = () => {
    ...
    const handleEditingBalance = () => {
        setIsBalanceUpdating(true);
        setTimeout(() => {
            setBalances((prev) => prev.map((balance) => balance.id === editBalanceId
    ...
    <Stack spacing={2} className='mx-10 lg:mx-44'>
      <Box>
        <IconButton onClick={() => onAddBalanceHandler()} sx={{ borderRadius: 2, justifyContent: 'flex-start' }}>
          <AttachMoneyIcon />
      ...
      <Box>
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label='List of balances' size='small'>
            ...
            return (<TableRow key={spaceBalance.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
      ...
      <Dialog open={addNewBalanceForm.tokenId !== ''} onClose={handleClose}>
        ...
        <DialogContent>
          ...
          <Stack spacing={2} pt={2}>
            <Box>
              <Select label='Token' fullWidth variant='standard' value={addNewBalanceForm.tokenId} onChange={(e) => {
            setAddNewBalanceForm((prev) => ({
                ...prev,
                tokenId: e.target.value,
            }));
        }}>
                {tokens.map((token) => (<MenuItem key={token.id} value={token.id}>
                    {token.ticker} [{token.name}]
                  </MenuItem>))}
              </Select>
            </Box>
            <TextField required label='Amount' type='number' fullWidth value={addNewBalanceForm.amount} onChange={(e) => {
            setAddNewBalanceForm((prev) => ({
                ...prev,
                amount: e.target.value,
            }));
        }}/>
            <TextField required label='Reason' fullWidth value={addNewBalanceForm.reason} onChange={(e) => {
            setAddNewBalanceForm((prev) => ({
                ...prev,
      ...
      <Dialog open={editBalanceId !== null} onClose={handleClose}>
        ...
        <DialogContent>
          ...
          <Stack spacing={2} pt={2}>
            <TextField required label='Amount' type='number' fullWidth value={editBalanceForm.amount} onChange={(e) => {
            setEditBalanceForm((prev) => ({
                ...prev,
                amount: e.target.value,
            }));
        }}/>
            <TextField required label='Reason' fullWidth value={editBalanceForm.reason} onChange={(e) => {
            setEditBalanceForm((prev) => ({
                ...prev,
                reason: e.target.value,
            }));
