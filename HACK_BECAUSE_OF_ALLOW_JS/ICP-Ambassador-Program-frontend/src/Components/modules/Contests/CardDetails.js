...
const CardDetails = () => {
    ...
    return (<div style={{
      ...
      <div className=' flex flex-col gap-8 justify-start items-start  lg:w-3/4 sm:w-full lg:p-0 sm:p-3  mt-10 h-full '>
        ...
        // <span>No Image</span>
        // </div>
        <img src='https://robots.net/wp-content/uploads/2023/11/what-is-blockchain-used-for-1698982380.jpg' alt={title} className="lg:w-44 lg:h-44 sm:w-44 sm:h-24 object-cover rounded-lg"/>)}
                </div>
            </div>
                        ...
                        <form className="flex flex-col gap-3 mt-3">
                            {task.id === 'SendText' && (<>
                                ...
                                {task.submitted ? (<div className='bg-[#1DB954] text-white flex gap-2 justify-center items-center py-2'>
                                  ...
                                  </div>) : (<div className={`text-white w-full py-2 flex justify-center items-center gap-2 rounded-md cursor-pointer ${task.content ? '' : 'opacity-40'}`} style={{ backgroundColor: bgColor }} onClick={() => {
                      ...
                      <img src={task.sampleImg} className='w-52 rounded h-40' alt=''/>
                                ...
                                {(task.uploading || task.image) && (<div className="w-full bg-[#1e1e1e] border border-[#FFFFFF14] rounded-md my-4 p-2">
                                    {task.uploading ? (<div className="w-full flex justify-between items-center gap-2 text-md font-semibold py-2 text-white">
                                      ...
                                      </div>) : task.image && !task.submitted ? (<div className="w-full flex justify-between items-center gap-2 text-md font-semibold py-2 text-white">
                                        <div className="flex gap-2">
                                          <AttachFileIcon />
                                ...
                                {task.image || task.submitted ? (null) : (<div className=' flex flex-col items-center mt-6  justify-center gap-2 text-white ' onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={(e) => handleDrop(e, task.task_id, task)}>
                            ...
                            {task.id === 'TwitterFollow' && (<div className='flex flex-col gap-4'>
                                <div className="text-white text-md max-w-[95%] overflow-clip">{`Task description :\n\n ${task.description}`}</div>
                                {task.submitted ?
                        ...
                        :
                            <div className='w-full text-white py-2 rounded-md flex gap-2 justify-center items-center text-lg' style={{ backgroundColor: bgColor }} onClick={() => submitTask(task.task_id)}>
                                    <FaXTwitter />
                                    <div>Follow</div> 
                                  </div>}
