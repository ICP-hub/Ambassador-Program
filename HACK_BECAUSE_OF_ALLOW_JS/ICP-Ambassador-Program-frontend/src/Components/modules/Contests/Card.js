    ...
    return (<div className=" text-white p-4 rounded-lg shadow-lg " style={{ backgroundColor: '#1d1d21' }} onClick={handleCard}>
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <span className={`text-sm py-1 px-2 rounded-md font-bold`} style={{
            backgroundColor: status === 'Active' ? '#1d2d27' :
                status === 'Draft' ? '#29292c' :
                    (status === null || status === undefined ? '#f0f0f0' : '#331a1e'),
            color: status === 'Active' ? '#1db851' :
                status === 'Draft' ? '#b8b8b8' :
                    (status === null || status === undefined ? '#a0a0a0' : '#e20203')
        }}>
  {typeof status === 'string' ? status : 'Unknown Status'}
    ...
    {social_platforms.map((platform, index) => {
      ...
      return (
        ...
        <div className="mb-4">
            {image ? (<img src={image} alt={title} className="w-24 h-24 object-cover rounded"/>) : (<div className="w-24 h-24 bg-gray-700 text-white flex items-center justify-center rounded">
                <span>No Image</span>
            </div>)}
        </div> 
      </div>
