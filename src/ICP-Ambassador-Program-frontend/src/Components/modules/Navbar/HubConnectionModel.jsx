...
const HubConnectionModal = ({ isOpen, onClose, spaces, setLoading }) => {
  ...
  const handleSelectChange = (e) => {
    ...
    const selectedSpace = spaces?.find(
      (space) => space.space_id === selectedSpaceId
    );
        ...
        {!verified ? (
          <>
            <h2 className="text-xl  mb-4">
              {joined ? "Verify if you have joined" : "Join Ambassador program"}
            </h2>
            {!joined ? (
              <button
              ...
              <>
                {verifying ? (
                  <p className="my-2 text-white text-xs font-semibold">
                    Verification in process...
                  </p>
                ) : (
                  <p className="my-2 text-white text-xs font-semibold">
                    Please Verify if you have joined our discord
                  </p>
                )}
                <button
                  disabled={verifying}
                  className="bg-black text-white px-4 py-2 rounded mt-4"
                  onClick={verifyGuildJoined}
                >
                  Verify
                </button>
              </>
