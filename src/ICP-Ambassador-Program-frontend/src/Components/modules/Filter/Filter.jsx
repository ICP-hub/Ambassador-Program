...
const Filter = () => {
      ...
      <div className="overflow-hidden h-full">
        <div className="p-2 mt-4 rounded-lg shadow-lg w-52" style={{backgroundColor:'#1d1d21'}}>
          <div className="flex justify-between items-center cursor-pointer" onClick={toggleFilter}>
            ...
            {isOpen ? (
              <IoChevronUpOutline className="text-white" style={{ fontSize: '20px' }} />
            ) : (
              <IoChevronDownOutline className="text-white" style={{ fontSize: '20px' }} />
            )}
          </div>
        ...
        <div className="p-2 mt-4 rounded-lg shadow-lg w-52" style={{backgroundColor:'#1d1d21'}}>
          ...
          <div
            className={`transition-all duration-500 ease-in-out overflow-hidden ${isOpenSort ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}
          >
            <div className="max-h-60 overflow-y-auto scrollbar-hide"> 
              <ul className="mt-4 space-y-4">
  ...
  function renderBox(id, label, icon) {
    return (
      <li className="flex items-center space-x-3 cursor-pointer" onClick={() => handleBoxClick(id)}>
        <div
          className={`w-5 h-5 flex items-center justify-center border border-gray-400 rounded-md transition-all duration-300 ${
            checkedItems[id] ? 'bg-white' : 'bg-transparent'
          }`}
        >
  ...
  function radiobutton(id, label) {
    return (
        <li className="flex items-center cursor-pointer" onClick={() => handleRadioChange(id)}>
        <div
          className={`w-4 h-4 rounded-full border-2 flex items-center justify-center mr-2 cursor-pointer transition-all duration-300 ${selectedRadio === id ? 'border-white bg-white' : 'border-gray-400'}`}
        >
